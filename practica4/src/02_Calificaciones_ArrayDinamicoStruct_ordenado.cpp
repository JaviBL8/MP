...
struct RegAlumno {
	char *DNI;
	double *notas;
	char *apellido_nombre; // Cadena "clásica"
};
...
void MuestraVectorAlumnos(RegAlumno alumnos[], double calificacion_media[], int total_alumnos){
	...
	{	
		cout<<alumnos[i].apellido_nombre<<"\t"<<alumnos[i].DNI<<"\t";
...
int LeerAlumnos (RegAlumno datos[],int num_casillas){
	...
	while(i<num_casillas){
		datos[i].apellido_nombre=new char [TAM_NOMBRE];
			
			cin.getline(datos[i].apellido_nombre,TAM_NOMBRE);
			if(datos[i].apellido_nombre[0]!='*'){
					
			strcat(datos[i].apellido_nombre," ");	
			char nombre[TAM_NOMBRE];
			cin.getline(nombre,TAM_NOMBRE);
			strcat(datos[i].apellido_nombre,nombre);
			datos[i].DNI=new char [TAM_DNI];
			cin.getline(datos[i].DNI,TAM_DNI);
			datos[i].notas= new double [NUM_NOTAS];
			for (int j = 0; j < NUM_NOTAS; ++j)
				{	
					char num[3];
					cin.getline(num,100);
